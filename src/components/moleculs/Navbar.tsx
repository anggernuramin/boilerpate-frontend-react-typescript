...
const Navbar: React.FC = () => {
  ...
  const ImageScrolled = () => {
    if (isScrolled) {
      return <span className="md:w-56 xs:w-40 -ms-4 md:-ms-0">logo</span>
    } else {
      return <span className="md:w-56 xs:w-40 -ms-4 md:-ms-0">Logo</span>
    }
  }
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-[9999] transition-all duration-300 ${classBgScrolled}`}>
      <div className="container flex items-center justify-between py-5 ">
        ...
        <FontAwesomeIcon
          className={`block w-6 h-6 md:hidden ${isScrolled ? 'text-black' : 'text-white'}`}
          onClick={toggleMenu}
          icon={faBars}
        />
        ...
        >
          ...
          <FontAwesomeIcon
            className="absolute w-6 h-6 text-white cursor-pointer top-5 right-5"
            onClick={toggleMenu}
            icon={faTimes}
          />
        </div>
